...
static IotclDiscoveryResponse *run_http_discovery(const char *cpid, const char *env) {
    ...
    //
    // dummy values, that one can free()
    //
    response->url = iotcl_strdup("dummy_url");
    response->host = iotcl_strdup("dummy_host");
    response->path = iotcl_strdup("dummy_path");
...
static IotclSyncResponse *run_http_sync(const char *cpid, const char *uniqueid) {
    ...
    cpid = cpid; // could use this?
    uniqueid = uniqueid; // not sure if this has cpid already added -- if not could use for client_id?
    ...
    if(platform_get_iotconnect_cpid(ret_str) == 0)
    {
        response->cpid = iotcl_strdup(ret_str);
    }
    else
    {
        IOTC_ERROR("platform_get_iotconnect_cpid failed.\n");
        return NULL;
    }

    response->dtg = iotcl_strdup("");
    response->ee = IOTCL_SR_OK;
    response->rc = IOTCL_SR_OK;
    response->at = IOTC_AT_X509;

    response->broker.name = iotcl_strdup("mqtt");
    if(platform_get_iotconnect_duid(ret_str) == 0)
    {
        response->broker.client_id = iotcl_strdup(ret_str);
    }
    else
    {
        IOTC_ERROR("platform_get_iotconnect_duid failed.\n");
        return NULL;
    }

    if(platform_get_mqtt_broker_ip(ret_str) == 0)
    {
        response->broker.host = iotcl_strdup(ret_str);
    }
    else
    {
        IOTC_ERROR("platform_get_mqtt_broker_ip failed.\n");
        return NULL;
    }

    if (platform_get_mqtt_broker_username(ret_str) == 0)
    {
        response->broker.user_name = iotcl_strdup(ret_str);
    }
    else
    {
        IOTC_ERROR("DA16X_CONF_STR_MQTT_USERNAME failed.\n");
        return NULL;
    }

    if (platform_get_mqtt_broker_password(ret_str) == 0)
    {
        response->broker.pass = iotcl_strdup(ret_str);
    }
    else
    {
        IOTC_ERROR("platform_get_mqtt_broker_password failed.\n");
        return NULL;
    }

    if (platform_get_mqtt_sub_topic0(ret_str) == 0)
    {
        response->broker.sub_topic = iotcl_strdup(ret_str);
    }
    else
    {
        IOTC_ERROR("platform_get_mqtt_sub_topic0 failed.\n");
        return NULL;
    }

    if (platform_get_mqtt_pub_topic(ret_str) == 0)
    {
        response->broker.pub_topic = iotcl_strdup(ret_str);
    }
    else
    {
        IOTC_ERROR("platform_get_mqtt_pub_topic failed.\n");
        return NULL;
    }
...
int iotconnect_sdk_run_mqtt_client(void) {
    ...
    if(pc.c2d_msg_cb == NULL)
    {
        IOTC_DEBUG("%s: pc.c2d_msg_cb == NULL\n" CLEAR_COLOR, __func__);
    }
    if(pc.status_cb == NULL)
    {
        IOTC_DEBUG("%s: pc.status_cb == NULL\n" CLEAR_COLOR, __func__);
    }
