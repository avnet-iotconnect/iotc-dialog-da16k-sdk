...
static _cJSON *setup_telemetry_object(IotclMessageHandle message) {
    ...
    if (!_cJSON_AddStringToObject(telemetry_object, "id", config->device.duid)) {
        goto cleanup_to;
    }
    if (!_cJSON_AddStringToObject(telemetry_object, "tg", "")) {
        goto cleanup_to;
    }
    ...
    if (!_cJSON_AddItemToArray(message->telemetry_data_array, telemetry_object)) {
        goto cleanup_da;
    }
    ...
    if (!_cJSON_AddItemToArray(data_array, message->current_telemetry_object)) {
        goto cleanup_cto;
    }
...
IotclMessageHandle iotcl_telemetry_create(void) {
    ...
    if (!_cJSON_AddStringToObject(msg->root_value, "cpid", config->device.cpid)) {
        goto cleanup_value;
    }
    if (!_cJSON_AddStringToObject(msg->root_value, "dtg", config->telemetry.dtg)) {
        goto cleanup_value;
    }
    if (!_cJSON_AddNumberToObject(msg->root_value, "mt", 0)) {
        goto cleanup_value; // telemetry message type (zero)
    }
    sdk_array = _cJSON_AddObjectToObject(msg->root_value, "sdk"); if (!sdk_array) {
        goto cleanup_value;
    }
    if (!_cJSON_AddStringToObject(sdk_array, "l", CONFIG_IOTCONNECT_SDK_NAME)) {
        goto cleanup_array;
    }
    if (!_cJSON_AddStringToObject(sdk_array, "v", CONFIG_IOTCONNECT_SDK_VERSION)) {
        goto cleanup_array;
    }
    if (!_cJSON_AddStringToObject(sdk_array, "e", config->device.env)) {
        goto cleanup_array;
    }
...
bool iotcl_telemetry_add_with_iso_time(IotclMessageHandle message, const char *time) {
    ...
    if (!_cJSON_HasObjectItem(message->root_value, "t")) {
        if (!_cJSON_AddStringToObject(message->root_value, "t", time)) {
            return false;
        }
    }
    return true;
