...
static const command_t commands[] = {
  ...
  { (char *)"AT+DPMTIMWU",        atcmd_dpm,            1, (char *) "<count>",                                    "Set DPM TIM Wakeup Time (DTIM count : 1 .. 65535)"     },
  { (char *)"AT+DPMUSERWU",       atcmd_dpm,            1, (char *) "<time>",                                     "Set DPM User Wakeup Time (sec : 0 .. 86400)"           },
#if defined (__SUPPORT_DPM_EXT_WU_MON__)
...
#endif // (__SUPPORT_DPM_EXT_WU_MON__)
  { (char *)"AT+SETSLEEP2EXT",    atcmd_dpm,            2, (char *) "<duration>,<rtm>",                           "Enter Sleep2 (sec : 1 .. 2,097,151)"                   },
  ...
  { (char *)"AT+WFENTAP",         atcmd_wifi,           5, (char *) "<ssid>,<auth>,<enc>,<phase1>[,<phase2>]",    "[STA] Set Enterprise AP profile"                       },
  ...
  { (char *)"AT+NWICSYNC",        atcmd_network,        1, (char *) "<0/1>",                                      "Start/stop IoTConnect sync"                            },
  { (char *)"AT+NWICDTG",         atcmd_network,        0, (char *) "<get_iotconnect_dtg>",                       "Get IoTConnect DTG (saved from discovery process)"     },
...
// might also relect device message version here?
  ...
  { (char *)"AT+NWICUSECMDACK",   atcmd_network,        1, (char *) "<0/1>",                                      "Enable/disable using CMDACK, i.e. responding to async commands"   },
  { (char *)"AT+NWICOTAACK",      atcmd_network,        4, (char *) "<ack_id>,<0/1>,<message>",                   "Acknowledge IoTConnect OTA with <ack_id>,etc."   },
  { (char *)"AT+NWICUSEOTAACK",   atcmd_network,        1, (char *) "<0/1>",                                      "Enable/disable using OTAACK, i.e. responding to async commands"   },
#endif

#if defined ( __SUPPORT_WPA_ENTERPRISE__ )
  { (char *)"AT+NWTLSV",          atcmd_network,        1, (char *) "<ver>",                                      "Set TLS Version"                                       },
#endif // __SUPPORT_WPA_ENTERPRISE__
  { (char *)"AT+NWCCRT",          atcmd_network,        0, (char *) "",                                           "Check whether Certificate exists (Root CA (bit 2) / Cert (bit 1) / Key (bit 0))" },
  ...
  { (char *)"AT+TRSSLCERTSTORE",  atcmd_transport_ssl,  5, (char *) "<cert_type>,<seq>,<format>,<name>,<data>",   "Store a certificate/CA list data."                     },
...
void atcmd_asynchony_event_for_icsync_start(void)
{
    PRINTF_ATCMD("\r\n+NWICSYNCSTART:\r\n");
}

void atcmd_asynchony_event_for_icsync_end(int status)
{
    PRINTF_ATCMD("\r\n+NWICSYNCEND:%d\r\n", status);
}

void atcmd_asynchony_event_for_icconfig_start(void)
{
    PRINTF_ATCMD("\r\n+NWICCONFIGSTART:\r\n");
}

void atcmd_asynchony_event_for_icconfig_end(int status)
{
    PRINTF_ATCMD("\r\n+NWICCONFIGEND:%d\r\n", status);
}
#endif
...
static void gets_tls_atcmd_uart(char *buf)
{
    ...
    char p_buf[32] = {0x00,};
    unsigned int p_idx = 0;
    atcmd_tls_cmd_parameter_step p_step = READ_CID;
    ...
    if (pos) {
        ...
        while (1) {
            ...
            if (p_step == READ_DATA) {
                ATCMD_DBG("[%s:%d]Read data with %s mode(%ld)\n", __func__, __LINE__,
                          r_mode ? "R" : "T", data_len);
...
atcmd_error_code command_parser(char *line)
{
    ...
    tmp_line = pvPortMalloc(length_all + 1);
    if (tmp_line == NULL) {
        PRINTF("[%s] Failed to allocate memory !!!\n", __func__);
        return AT_CMD_ERR_MEM_ALLOC;
    }
    memset(tmp_line, 0, length_all + 1);
...
int atcmd_dpm(int argc, char *argv[])
{
    ...
    // AT+SETSLEEP2EXT
        ...
        if (argc == 3) {
            unsigned long long tmp_sec = 0x0ULL;

            /* AT+SETSLEEP2EXT=<period>,<rtm> */
            if (get_int_val_from_str(argv[1], &tmp_int1, POL_1) != 0) {
                err_code = AT_CMD_ERR_DPM_SLP2_PERIOD_TYPE;
                goto atcmd_dpm_end;
            } else if (is_in_valid_range(tmp_int1, 0, 0x1FFFFF) == pdFALSE) {
                err_code = AT_CMD_ERR_DPM_SLP2_PERIOD_RANGE;
                goto atcmd_dpm_end;
            } else {
                tmp_sec = (unsigned long long)tmp_int1;
            }
...
#if defined (__BLE_COMBO_REF__)
            result_int = combo_set_sleep2("at", tmp_int2, tmp_int1);
            if (result_int < 0) {
                if (result_int == -1) {
            ...
            dpm_sleep_start_mode_2(tmp_sec * 1000000ULL, tmp_int2);
...
#define ATCMD_WIFI_VALUE_STR_LEN    128
int atcmd_wifi(int argc, char *argv[])
{
    int  err_code = AT_CMD_ERR_CMD_OK;
    char value_str[ATCMD_WIFI_VALUE_STR_LEN] = {0, };
    int  value_int = 0;
    int  tmp_int1 = 0;
#if defined ( __SUPPORT_WIFI_CONCURRENT__ )
    int  status;
    ...
    // AT+WFPSCAN
        ...
        char cmd[ATCMD_WIFI_VALUE_STR_LEN + 1] = {0, };
        ...
        if (argc < 3) {        // WFPSCAN, interval, freq_1, freq_2,,,freq_14
            ...
            if (atoi(argv[2]) != 0) {
                ...
                for (int i = 2; i < argc; i++ ) {
                    ...
                    if(isDuplicate[atoi(argv[i])] == true) // check duplicate channel
                    { 
                        err_code = AT_CMD_ERR_WRONG_ARGUMENTS;
                        goto atcmd_wifi_end;
                    } else {
                        isDuplicate[atoi(argv[i])] = true;
                    }
                    if(chk_channel_by_country(country, i3ed11_ch_to_freq(atoi(argv[i]), 0), 1, NULL) < 0) // check country support channel
                    {
                        err_code = AT_CMD_ERR_WIFI_PSCAN_FREQ_RANGE;
                        goto atcmd_wifi_end;
                    }
    ...
    // AT+WFJAP
        ...
        if (argc == 1 || is_correct_query_arg(argc, argv[1])) {
            ...
            */
            ...
            value_int = printf_decode((u8 *) value_str, (ATCMD_WIFI_VALUE_STR_LEN-1), argv[1]);
            ...
            if (auth_mode == CC_VAL_AUTH_WEP) {
                ...
                if (auth_mode == CC_VAL_AUTH_SAE || auth_mode == CC_VAL_AUTH_RSN_SAE) {
                    ...
                    da16x_set_nvcache_str(DA16X_CONF_STR_SAE_PASS_0, argv[4]);
                } else {
                    da16x_set_nvcache_int(DA16X_CONF_INT_ENCRYPTION_0, atoi(argv[3])); // <enc>
                    da16x_set_nvcache_str(DA16X_CONF_STR_PSK_0, argv[4]); // <key>
                }
#else
                da16x_set_nvcache_int(DA16X_CONF_INT_ENCRYPTION_0, atoi(argv[3])); // <enc>
                da16x_set_nvcache_str(DA16X_CONF_STR_PSK_0, argv[4]); // <key>
            ...
            if (is_ssid_hidden == pdTRUE) {
                ret = da16x_cli_reply("set_network 0 scan_ssid 1", NULL, value_str);
                if (ret < 0 || strcmp(value_str, "FAIL") == 0) {
                    err_code = AT_CMD_ERR_WIFI_CLI_SET_NETWORK_HIDDEN;
                    goto atcmd_wifi_end;
                }

                ret = da16x_cli_reply("save_config", NULL, value_str);
                if (ret < 0 || strcmp(value_str, "FAIL") == 0) {
                    err_code = AT_CMD_ERR_WIFI_CLI_SAVE_CONF_HIDDEN;
                    goto atcmd_wifi_end;
                }
            }
    ...
    // AT+WFJAPA
        ...
        if (argc == 1 || is_correct_query_arg(argc, argv[1])) {
            ...
            */
            ...
            /* Support Extended ASCII or UTF-8 for SSID
             *    Do not use the argv[1] string directly. */
            value_int = printf_decode((u8 *) value_str, (ATCMD_WIFI_VALUE_STR_LEN-1), ssid);
            if (value_int > MAX_SSID_LEN) {
        ...
        if (argc == 1 || is_correct_query_arg(argc, argv[1])) {
            ...
            */
            ...
            case WPA3_HIDDEN:
                ...
                //cli set_network 0 sae_password *********
                da16x_set_nvcache_str(DA16X_CONF_STR_SAE_PASS_0, psk);
                break;
    ...
    // AT+WFENTAP
        ...
        if (argc == 1 || is_correct_query_arg(argc, argv[1])) {
            ...
            if (da16x_get_config_int(DA16X_CONF_INT_EAP_PHASE2, &p2)) {
            ...
            if ((wpa < CC_VAL_AUTH_OPEN || wpa > CC_VAL_AUTH_WPA_AUTO_EAP)) { // Wi-Fi Auth mode
                err_code = AT_CMD_ERR_WIFI_ENTAP_SECU_MODE;
                goto atcmd_wifi_end;
            ...
            if ((argc == 6) && (p2 < CC_VAL_EAP_PHASE2_MIX || p2 > CC_VAL_EAP_GTC)) { // EAP Phase #2
                err_code = AT_CMD_ERR_WIFI_ENTAP_EAP_PHASE2;
                goto atcmd_wifi_end;
            }
        ...
        if (argc == 1 || is_correct_query_arg(argc, argv[1])) {
            ...
            */
            ...
            if (auth_mode < 0
                ...
                err_code = AT_CMD_ERR_WIFI_SOFTAP_ENC_MODE;
                goto atcmd_wifi_end;
            }

            // sanity check: <ch>, <country>
            if ((auth_mode == CC_VAL_AUTH_OPEN && argc >= 4) || (auth_mode > CC_VAL_AUTH_WEP && argc >= 6)) {
                /*
                ...
                if (!tmp) {
                    err_code = AT_CMD_ERR_WIFI_SOFTAP_CH_TX_PWR_VALUE;
                    goto atcmd_wifi_end;
                }
            }
            ...
            if (auth_mode == CC_VAL_AUTH_WEP) {
                ...
                if (get_int_val_from_str(argv[3], &enc_arg, POL_1) != 0) {
                    err_code = AT_CMD_ERR_WIFI_SOFTAP_ENC_MODE_TYPE;
                    goto atcmd_wifi_end;
                }

                if (enc_arg < 0 || enc_arg > CC_VAL_ENC_AUTO) {
            ...
            if (   auth_mode == CC_VAL_AUTH_OPEN
                ...
                da16x_set_nvcache_int(DA16X_CONF_INT_ENCRYPTION_1, tmp_int1);
                // <key>
#if defined ( __SUPPORT_WPA3_PERSONAL__ )
                if (auth_mode == CC_VAL_AUTH_SAE)
                    da16x_set_nvcache_str(DA16X_CONF_STR_SAE_PASS_1, argv[4]);
                else
#endif // __SUPPORT_WPA3_PERSONAL__
                    da16x_set_nvcache_str(DA16X_CONF_STR_PSK_1, argv[4]);
...
int atcmd_network(int argc, char *argv[])
{
...
#if defined ( __OTA_UPDATE_MCU_FW__ )
  #if defined  (__SUPPORT_UART1__) || defined (__ATCMD_IF_UART1__)
    int index = UART_UNIT_1;    // Default UART interface for AT-CMD
  #elif defined  (__SUPPORT_UART2__) || defined (__ATCMD_IF_UART2__)
    int index = UART_UNIT_2;
  #endif //( __SUPPORT_UART1__ ) ||( __SUPPORT_UART2__ )||(__ATCMD_IF_UART1__) || (__ATCMD_IF_UART2__)
#endif // __OTA_UPDATE_MCU_FW__
...
#if 1
    // AT+NWICSYNC
    else if (strcasecmp(argv[0] + 5, "ICSYNC") == 0) {
        if (argc == 1 || is_correct_query_arg(argc, argv[1])) {
            /* AT+NWICSYNC=? */
            da16x_get_config_str(DA16X_CONF_STR_IOTCONNECT_SYNC_MODE, result_str);
        } else if (argc == 2) {
            /* AT+NWICSYNC=<0/1> */
            if (da16x_set_config_str(DA16X_CONF_STR_IOTCONNECT_SYNC_MODE, argv[1]) != CC_SUCCESS) {
                err_code = AT_CMD_ERR_COMMON_ARG_LEN;
            }
        } else {
            err_code = AT_CMD_ERR_INSUFFICENT_ARGS;
    ...
    // AT+NWICUSECMDACK
        ...
        if (argc == 1 || is_correct_query_arg(argc, argv[1])) {
            /* AT+NWICCMDACK=? */
            da16x_get_config_str(DA16X_CONF_STR_IOTCONNECT_USE_CMDACK, result_str);
        } else if (argc == 2) {
            /* AT+NWICUSECMDACK=<0/1> */
            if (da16x_set_config_str(DA16X_CONF_STR_IOTCONNECT_USE_CMDACK, argv[1]) != CC_SUCCESS) {
                err_code = AT_CMD_ERR_COMMON_ARG_LEN;
            }
        } else {
            err_code = AT_CMD_ERR_INSUFFICENT_ARGS;
    ...
    // AT+NWICUSEOTAACK
        ...
        if (argc == 1 || is_correct_query_arg(argc, argv[1])) {
            /* AT+NWICOTAACK=? */
            da16x_get_config_str(DA16X_CONF_STR_IOTCONNECT_USE_OTAACK, result_str);
        } else if (argc == 2) {
            /* AT+NWICUSEOTAACK=<0/1> */
            if (da16x_set_config_str(DA16X_CONF_STR_IOTCONNECT_USE_OTAACK, argv[1]) != CC_SUCCESS) {
                err_code = AT_CMD_ERR_COMMON_ARG_LEN;
            }
        } else {
            err_code = AT_CMD_ERR_INSUFFICENT_ARGS;
    ...
    // AT+NWICDTG
    else if (strcasecmp(argv[0] + 5, "ICDTG") == 0) {
        if (argc == 1 || is_correct_query_arg(argc, argv[1])) {
            /* AT+NWICDTG=? */
            da16x_get_config_str(DA16X_CONF_STR_IOTCONNECT_DTG, result_str);
        } else {
            err_code = AT_CMD_ERR_INSUFFICENT_ARGS;
        }
    }
    ...
    // AT+NWICMSG
        ...
        if (argc == 3 || argc == 5 || argc == 7 || argc == 9 || argc == 11 || argc == 13 || argc == 15) {
            ...
            if(str == NULL) {
                ATCMD_DBG("iotcl_serialise_telemetry_strings() failed\n");
                err_code = AT_CMD_ERR_NOT_SUPPORTED;
            } else {
...
#define IOTCONNECT_AT_VERSION "1.0.0"
    ...
    // AT+NWICOTAACK
        ...
        if (argc == 4) {
           ...
           bool cmd_status = (bool) atoi(argv[2]);
           const char *message = (const char *) argv[3];

        PRINTF("ICOTAACK ack_id %s\n", ack_id);
        PRINTF("ICOTAACK cmd_status %d\n", cmd_status);
        PRINTF("ICOTAACK message %s\n", message);

           iotconnect_ota_status(ack_id, cmd_status, message);
        } else {
            err_code = AT_CMD_ERR_INSUFFICENT_ARGS;
        }
    }
    ...
    // AT+NWCCRT
        ...
        // Need to check buffer alloc fail status
        if (buffer == NULL) {
            err_code = AT_CMD_ERR_MEM_ALLOC;
            goto atcmd_network_end;
        }
        ...
        // cert #1
        cert_flash_read(SFLASH_ROOT_CA_ADDR1, buffer, CERT_MAX_LENGTH);
        if ((UCHAR)(buffer[0]) != 0xFF) {
            chk |= (1 << 2);
        }

        cert_flash_read(SFLASH_CERTIFICATE_ADDR1, buffer, CERT_MAX_LENGTH);
        if ((UCHAR)(buffer[0]) != 0xFF) {
            chk |= (1 << 1);
        }

        cert_flash_read(SFLASH_PRIVATE_KEY_ADDR1, buffer, CERT_MAX_LENGTH);
        if ((UCHAR)(buffer[0]) != 0xFF) {
            chk |= 1;
        }

        // cert #2
        cert_flash_read(SFLASH_ROOT_CA_ADDR2, buffer, CERT_MAX_LENGTH);
        if ((UCHAR)(buffer[0]) != 0xFF) {
            chk |= (1 << 5);
        }

        cert_flash_read(SFLASH_CERTIFICATE_ADDR2, buffer, CERT_MAX_LENGTH);
        if ((UCHAR)(buffer[0]) != 0xFF) {
            chk |= (1 << 4);
        }

        cert_flash_read(SFLASH_PRIVATE_KEY_ADDR2, buffer, CERT_MAX_LENGTH);
        if ((UCHAR)(buffer[0]) != 0xFF) {
            chk |= (1 << 3);
        }
    ...
    || defined (__ATCMD_IF_UART1__) || defined (__ATCMD_IF_UART2__)
                UINT addr, size;
                char *end = NULL;
                addr = strtol(argv[1], &end, 16);
                ...
                if (ota_update_uart_read_mcu_fw(addr, size) != OTA_SUCCESS) {
                    sprintf(result_str, "%s", "FAIL");
                } else {
                    sprintf(result_str, "%s", "COMPLETE");
                }
...
#if defined ( __SUPPORT_UART1__ ) || defined ( __SUPPORT_UART2__ ) \
    || defined (__ATCMD_IF_UART1__) || defined (__ATCMD_IF_UART2__)
                if (ota_update_uart_init(index)) {
                    sprintf(result_str, "%s", "FAIL");
                } else {
                    if (ota_update_uart_trans_mcu_fw()) {
                        sprintf(result_str, "%s", "FAIL");
                    } else {
                        sprintf(result_str, "%s", "COMPLETE");
                    }
...
static int atcmd_transport_ssl_do_certstore(char *type_str, char *seq_str, char *format_str, char *name_str, char *data)
{
    int status = DA_APP_SUCCESS;
    ...
    status = atcmd_cm_set_cert(name_str, (unsigned char)type, (unsigned char)seq,
                               (unsigned char)format, data, (strlen(data) + 1));
    if (status) {
        ATCMD_DBG("[%s:%d]failed to set certificate(name: %s(%ld), type: %d, cert: (%ld)\n",
                  __func__, __LINE__, name_str, strlen(name_str), type, strlen(data));
        return status;
    }
...
int atcmd_transport_ssl(int argc, char *argv[])
{
    ...
    //AT+TRSSLCERTSTORE
    else if (strcasecmp(argv[0] + 8, "CERTSTORE") == 0) {
        if (argc == 6) {
            // Certificate-type
            if (get_int_val_from_str(argv[1], &tmp_int1, POL_1) != 0) {
                err_code = AT_CMD_ERR_SSL_CERT_TYPE;
            ...
            // Sequence
            if (get_int_val_from_str(argv[2], &tmp_int1, POL_1) != 0) {
                err_code = AT_CMD_ERR_SSL_CERT_STO_SEQ_TYPE;
                goto transport_ssl_end;
            }
            if (is_in_valid_range(tmp_int1, 0, 5) == pdFALSE) {
                err_code = AT_CMD_ERR_SSL_CERT_STO_SEQ_RANGE;
                goto transport_ssl_end;
            }

            // Format
            if (get_int_val_from_str(argv[3], &tmp_int1, POL_1) != 0) {
                err_code = AT_CMD_ERR_SSL_CERT_STO_FORMAT_TYPE;
                goto transport_ssl_end;
            }
            if (is_in_valid_range(tmp_int1, 0, 1) == pdFALSE) {
                err_code = AT_CMD_ERR_SSL_CERT_STO_FORMAT_RANGE;
                goto transport_ssl_end;
            }

            err_code = AT_CMD_ERR_CMD_OK;

            status = atcmd_transport_ssl_do_certstore(argv[1], argv[2], argv[3], argv[4], argv[5]);
            switch (status) {
            case DA_APP_SUCCESS :
                err_code = AT_CMD_ERR_CMD_OK;
                break;
            ...
            case DA_APP_DUPLICATED_ENTRY :
                err_code = AT_CMD_ERR_SSL_CERT_STO_ALREADY_EXIST;
                break;

            case DA_APP_OVERFLOW :
                err_code = AT_CMD_ERR_SSL_CERT_STO_NO_SPACE;
                break;
...
{
    char ip_str[16] = { 0, };
    const int wait_count_initial_value = 30; // Sync with #define MAX_INIT_WIFI_CONN_TIME 30
    int  wait_count = wait_count_initial_value;
    ...
    while (1) {
        ...
        if (    wait_count == (wait_count_initial_value/2)
            && wifi_conn_fail_reason_code == 45 /* WLAN_REASON_PEERKEY_MISMATCH */ ) {

            // wait for half the wait time if WRONGKEY is the cause
            wait_count = 1;
        }
    }
}
