...
extern int cert_flash_read_no_fopen(HANDLE flash_handler, int flash_addr, char *result, size_t len);
...
int mqtt_client_cert_read(void)
{
	...
	if (cert_flash_read_no_fopen(flash_handler, SFLASH_ROOT_CA_ADDR1, mqtt_cfg.cacert_ptr, CERT_MAX_LENGTH) != 0)
	{
		PRINTF("[%s:%d] CA Read error\n", __func__, __LINE__);
		return pdFAIL;
	}
	
	if (cert_flash_read_no_fopen(flash_handler, SFLASH_CERTIFICATE_ADDR1, mqtt_cfg.cert_ptr, CERT_MAX_LENGTH) != 0)
	{
		PRINTF("[%s:%d] Client Cert Read error\n", __func__, __LINE__);
		return pdFAIL;
	}

	if (cert_flash_read_no_fopen(flash_handler, SFLASH_PRIVATE_KEY_ADDR1, mqtt_cfg.priv_key_ptr, CERT_MAX_LENGTH) != 0)
	{
		PRINTF("[%s:%d] Client Key Read error\n", __func__, __LINE__);
		return pdFAIL;
	}
...
int mqtt_client_save_to_cfg(void)
{
	...
	{
		memset(mqtt_cfg.cacert_ptr, 0, CERT_MAX_LENGTH);
	}
	if (mqtt_cfg.cert_ptr)
	{
		memset(mqtt_cfg.cert_ptr, 0, CERT_MAX_LENGTH);
	}
	if (mqtt_cfg.priv_key_ptr)
	{
		memset(mqtt_cfg.priv_key_ptr, 0, CERT_MAX_LENGTH);
	}
	...
	if (mqtt_cfg.insecure == 1)
	{
		...
		{
			mqtt_cfg.cacert_ptr = _mosquitto_malloc(CERT_MAX_LENGTH);
		}
		if (mqtt_cfg.cert_ptr == NULL)
		{
			mqtt_cfg.cert_ptr = _mosquitto_malloc(CERT_MAX_LENGTH);
		}
		if (mqtt_cfg.priv_key_ptr == NULL)
		{
			mqtt_cfg.priv_key_ptr = _mosquitto_malloc(CERT_MAX_LENGTH);
		}
		
		// read certificates
		if (mqtt_client_cert_read() != pdPASS) return pdFALSE;

		if ((UCHAR)(mqtt_cfg.cacert_ptr[0]) == 0xFF)
		{
			_mosquitto_free(mqtt_cfg.cacert_ptr);
			mqtt_cfg.cacert_ptr = NULL;
		}
		if ((UCHAR)(mqtt_cfg.cert_ptr[0]) == 0xFF)
		{
			_mosquitto_free(mqtt_cfg.cert_ptr);
			mqtt_cfg.cert_ptr = NULL;
		}
		if ((UCHAR)(mqtt_cfg.priv_key_ptr[0]) == 0xFF)
		{
...
int client_opts_set(struct mosquitto *mosq, struct mosq_config *cfg)
{
	...
	if (cfg->insecure)
	{
		...
		if (cfg->cacert_ptr)
		{
			if (mosquitto_tls_ca_cert_set(mosq, cfg->cacert_ptr, strlen(cfg->cacert_ptr) + 1))
			{
		...
		if (cfg->cert_ptr)
		{
			if (mosquitto_tls_cert_set(mosq, cfg->cert_ptr, strlen(cfg->cert_ptr) + 1))
			{
		...
		if (cfg->priv_key_ptr)
		{
			if (mosquitto_tls_private_key_set(mosq, cfg->priv_key_ptr, strlen(cfg->priv_key_ptr) + 1))
			{
				MQTT_DBG_ERR(RED_COLOR "Failed to set cert\n" CLEAR_COLOR);
				return 1;
			}
...
static void print_usage(void)
{
	...
	PRINTF("\tmqtt_config long_username \r\n");
...
UINT mqtt_client_config(int argc, char *argv[])
{
	...
	else if (strcmp(argv[1], "long_username") == 0 && argc == 2)
	{
		...
		if (buffer == NULL)
		{
			PRINTF("[%s] Failed to alloc memory for username\n", __func__);
			status = MOSQ_ERR_NOMEM;
		}
		ret = make_message("username", buffer, MQTT_USERNAME_MAX_LEN + 1);
		PRINTF("\n");
		if (ret > 0)
		{
			status = da16x_set_config_str(DA16X_CONF_STR_MQTT_USERNAME, buffer);
			if (status)
			{
...
next_mqtt:
	...
	rtc_register_timer(5, "MQTT_TEMP", 0, 0, NULL);
