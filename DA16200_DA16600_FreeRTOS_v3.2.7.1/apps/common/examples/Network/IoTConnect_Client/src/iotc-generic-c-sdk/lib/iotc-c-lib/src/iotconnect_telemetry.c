...
static _cJSON *setup_telemetry_object(IotclMessageHandle message) {
    ...
    _cJSON *telemetry_object = _cJSON_CreateObject();
    if (!telemetry_object) return NULL;
    if (!_cJSON_AddStringToObject(telemetry_object, "id", config->device.duid)) goto cleanup_to;
    if (!_cJSON_AddStringToObject(telemetry_object, "tg", "")) goto cleanup_to;
    _cJSON *data_array = _cJSON_AddArrayToObject(telemetry_object, "d");
    if (!data_array) goto cleanup_to;
    if (!_cJSON_AddItemToArray(message->telemetry_data_array, telemetry_object)) goto cleanup_da;
    ...
    if (!_cJSON_AddItemToArray(data_array, message->current_telemetry_object)) goto cleanup_cto;
    ...
    cleanup_da:
    _cJSON_free(data_array);

    cleanup_to:
    _cJSON_free(telemetry_object);

    return NULL;
}

IotclMessageHandle iotcl_telemetry_create(void) {
    _cJSON *sdk_array = NULL;
    IotclConfig *config = iotcl_get_config();
    if (!config) {
        return NULL;
    }
    ...
    msg->root_value = _cJSON_CreateObject();

    if (!msg->root_value) goto cleanup;

    if (!_cJSON_AddStringToObject(msg->root_value, "cpid", config->device.cpid)) goto cleanup_value;
    if (!_cJSON_AddStringToObject(msg->root_value, "dtg", config->telemetry.dtg)) goto cleanup_value;
    if (!_cJSON_AddNumberToObject(msg->root_value, "mt", 0)) goto cleanup_value; // telemetry message type (zero)
    sdk_array = _cJSON_AddObjectToObject(msg->root_value, "sdk");
    if (!sdk_array) goto cleanup_value;
    if (!_cJSON_AddStringToObject(sdk_array, "l", CONFIG_IOTCONNECT_SDK_NAME)) goto cleanup_array;
    if (!_cJSON_AddStringToObject(sdk_array, "v", CONFIG_IOTCONNECT_SDK_VERSION)) goto cleanup_array;
    if (!_cJSON_AddStringToObject(sdk_array, "e", config->device.env)) goto cleanup_array;

    msg->telemetry_data_array = _cJSON_AddArrayToObject(msg->root_value, "d");
    ...
    cleanup_array:
    _cJSON_Delete(sdk_array);

    cleanup_value:
    _cJSON_Delete(msg->root_value);
...
bool iotcl_telemetry_add_with_iso_time(IotclMessageHandle message, const char *time) {
    ...
    if (!_cJSON_AddStringToObject(telemetry_object, "dt", time)) return false;
    if (!_cJSON_HasObjectItem(message->root_value, "t")) {
        if (!_cJSON_AddStringToObject(message->root_value, "t", time)) return false;
    }
    return true;
}
...
bool iotcl_telemetry_set_number(IotclMessageHandle message, const char *path, double value) {
    ...
    if (!_cJSON_AddNumberToObject(target, leaf_name, value)) goto cleanup_tgt;
    free(leaf_name);
    return true;
...
bool iotcl_telemetry_set_bool(IotclMessageHandle message, const char *path, bool value) {
    ...
    if (!_cJSON_AddBoolToObject(target, leaf_name, value)) goto cleanup_tgt;
    free(leaf_name);
    return true;
...
bool iotcl_telemetry_set_string(IotclMessageHandle message, const char *path, const char *value) {
    ...
    if (!_cJSON_AddStringToObject(target, leaf_name, value)) goto cleanup_tgt;
    free(leaf_name);
    return true;
...
bool iotcl_telemetry_set_null(IotclMessageHandle message, const char *path) {
    ...
    if (!_cJSON_AddNullToObject(target, leaf_name)) goto cleanup_tgt;
    free(leaf_name);
    return true;
...
const char *iotcl_create_serialized_string(IotclMessageHandle message, bool pretty) {
    ...
    serialized_string = (pretty) ? _cJSON_Print(message->root_value) : _cJSON_PrintUnformatted(message->root_value);
