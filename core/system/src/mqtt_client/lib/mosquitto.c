...
{
	enum mosquitto_client_state prev_state = mosq->state;
...
int mosquitto_loop(struct mosquitto *mosq, int timeout, int max_packets)
{
	...
	if (mosq->state == mosq_cs_disconnecting || 
		da16x_network_main_check_net_link_status(WLAN0_IFACE) == pdFALSE)
	{
...
UINT mosquitto_pingreq_timer_register(struct mosquitto *mosq)
{
	...
	else
	{
		if (mosq->mqtt_pub_or_sub == CLIENT_SUB)
		{
			MQTT_DBG_INFO(GREEN_COLOR " [%s] Regist keepalivetime(id=%d, itv=%d).\n" CLEAR_COLOR,
						  __func__, MQTT_SUB_PERIOD_TIMER_ID, mosq->keepalive);
			status = rtc_register_timer(mosq->keepalive, APP_MQTT_SUB, MQTT_SUB_PERIOD_TIMER_ID, 1, mosquitto_sub_pingreq_timer_cb);
		}
		else
		{
			MQTT_DBG_ERR(RED_COLOR "Failed to regist keepalivetime(pub_or_sub=%d).\n" CLEAR_COLOR,
...
void create_mosquitto_pingreq_sub_thread(void)
{
	...
	// state = (eDeleted / eInvalid) or (mosquittoPingSubThread == NULL)
