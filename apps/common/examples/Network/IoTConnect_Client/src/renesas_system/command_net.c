...
static void cmd_iotconnect_config_usage(void)
{
    ...
    PRINTF("  Usage : iotconnect_config [status | reset | connection_type | env | cpid | duid | symmetric_key | auth_type | use_cmd_ack | use_ota_ack] [value]\n");
}

void cmd_iotconnect_config(int argc, char *argv[])
{
    if (argc == 1) {
        cmd_iotconnect_config_usage();
        cmd_iotconnect_config_status();
    } else if (argc == 2) {
        if(strcmp(argv[1], "status") == 0) {
            cmd_iotconnect_config_status();
        }
        else if(strcmp(argv[1], "reset") == 0) {
            if (da16x_set_config_str(DA16X_CONF_STR_IOTCONNECT_ENV, "") != CC_SUCCESS)
            {
                PRINTF("Failed to reset env");
            }
            ...
            {
                PRINTF("Failed to reset duid");
            }
            if (da16x_set_config_str(DA16X_CONF_STR_IOTCONNECT_SYMMETRIC_KEY, "") != CC_SUCCESS)
            {
                PRINTF("Failed to reset symmetric key");
            }
            if (da16x_set_config_str(DA16X_CONF_STR_IOTCONNECT_AUTH_TYPE, "") != CC_SUCCESS)
            {
                PRINTF("Failed to reset authentication type");
            }
            if (da16x_set_config_str(DA16X_CONF_STR_IOTCONNECT_USE_CMD_ACK, "") != CC_SUCCESS)
            {
                PRINTF("Failed to reset use command acknowledgement");
            }
            if (da16x_set_config_str(DA16X_CONF_STR_IOTCONNECT_USE_OTA_ACK, "") != CC_SUCCESS)
            {
        ...
        if(strcmp(argv[1], "connection_type") == 0) {
        ...
        } else if(strcmp(argv[1], "cpid") == 0) {
            if (da16x_set_config_str(DA16X_CONF_STR_IOTCONNECT_CPID, argv[2]) != CC_SUCCESS)
            {
            ...
            {
                PRINTF("Failed to set duid");
            }
        } else if(strcmp(argv[1], "symmetric_key") == 0) {
            if (da16x_set_config_str(DA16X_CONF_STR_IOTCONNECT_SYMMETRIC_KEY, argv[2]) != CC_SUCCESS)
            {
                PRINTF("Failed to set symmetric key");
            }
        } else if(strcmp(argv[1], "auth_type") == 0) {
            if (da16x_set_config_str(DA16X_CONF_STR_IOTCONNECT_AUTH_TYPE, argv[2]) != CC_SUCCESS)
            {
                PRINTF("Failed to set authentication type");
            }
        } else if(strcmp(argv[1], "use_cmd_ack") == 0) {
            if (da16x_set_config_str(DA16X_CONF_STR_IOTCONNECT_USE_CMD_ACK, argv[2]) != CC_SUCCESS)
            {
                PRINTF("Failed to set use command acknowledgement");
            }
        } else if(strcmp(argv[1], "use_ota_ack") == 0) {
            if (da16x_set_config_str(DA16X_CONF_STR_IOTCONNECT_USE_OTA_ACK, argv[2]) != CC_SUCCESS)
            {
                PRINTF("Failed to set use OTA acknowledgement");
            }
        } else {
            goto usage;
        }
    ...
    if (argc == 2 && strcmp(argv[1], "stop") == 0) {
        ...
        reset_iotconnect();
         ...
         if (argc > 2) {
            ...
            if(argc != 2 + (half_argc * 2)) {
                MQTT_DBG_PRINT("iotconnect_client msg needs an even number of 'name' 'value' pairs as its arguments\n");
                return;
            }
...
usage:
    PRINTF("- iotconnect_client\n\n");
    PRINTF("  Usage : iotconnect_client setup (either on first run, or after \"iotconnect_client reset\")\n");
    PRINTF("          iotconnect_client start (either after \"iotconnect_client setup\" or after \"iotconnect_client stop\")\n");
    PRINTF("          iotconnect_client stop (after \"iotconnect_client start\" to stop)\n");
    PRINTF("          iotconnect_client reset (after \"iotconnect_client start\" to force settings to reset)\n");
    PRINTF("\n");
    PRINTF("          iotconnect_client msg name value [name2 value2] [name3 value3] ...\n");
    PRINTF("          iotconnect_client cmd_ack type ack_id 0 message\n");
    PRINTF("          iotconnect_client cmd_ack type ack_id 1 message\n");
